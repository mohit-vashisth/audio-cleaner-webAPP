...
def denoise_audio(info: AudioInfo) -> AudioInfo:
    try:
        ...
        sources = apply_model(model, mix=audio, split=True, progress=False)
        vocals = sources[0][0]  # Assuming vocals is the first source
        return AudioInfo(vocals, info.sr)

    except Exception as e:
        print(f"[Fallback to noisereduce] Demucs error: {e}")

        # Original noisereduce fallback
...
async_pipeline_audio(
    r"C:\Users\Admin\Documents\audio-cleaner-webAPP\backend\uploads\sample.mp3",
))